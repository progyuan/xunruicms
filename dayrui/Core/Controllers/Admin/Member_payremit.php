...
// 汇款管理
class Member_payremit extends \Phpcmf\Table
{
    ...
    public function __construct(...$params)
    {
        ...
        $this->_init([
            ...
            'where_list' => '`type`="remit"',
        ]);
        \Phpcmf\Service::V()->assign([
            'menu' => \Phpcmf\Service::M('auth')->_admin_menu(
                [
                    '汇款管理' => [\Phpcmf\Service::L('Router')->class.'/index', 'fa fa-cc-visa'],
    ...
    protected function _Save($id = 0, $data = [], $old = [], $before = null, $after = null) {
        ...
        // 更新提醒
        \Phpcmf\Service::M('member')->todo_admin_notice('member_payremit/edit:id/'.$id, 0);

        // 通过打款
        if ($post['verify']) {
            \Phpcmf\Service::M('member')->notice($old['uid'], 5, '流水号('.$id.') 转账汇款已成功',\Phpcmf\Service::L('Router')->member_url('paylog/show', ['id'=>$id]));
            return \Phpcmf\Service::M('Pay')->paysuccess('fc-'.$id, $post['note']);
        } else {
            \Phpcmf\Service::M('member')->notice($old['uid'], 5, '流水号('.$id.') 转账汇款失败',\Phpcmf\Service::L('Router')->member_url('paylog/show', ['id'=>$id]));
