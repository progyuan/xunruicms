...
class Module extends \Phpcmf\Common
{
    ...
    protected function _Index_Html() {
        ...
        $this->_Index(1);
        $html = ob_get_clean();
        $pc = file_put_contents(\Phpcmf\Service::L('html')->get_webpath(SITE_ID, $this->module['dirname'], $file), $html, LOCK_EX);
        ...
        if (SITE_IS_MOBILE) {
            ...
            $this->_Index(1);
            $html = ob_get_clean();
            $mobile = file_put_contents(\Phpcmf\Service::L('html')->get_webpath(SITE_ID, $this->module['dirname'], 'mobile/' . $file), $html, LOCK_EX);
        }
